...
class pixo_connect : public pixpg::db {
  public:
    void send(const std::string& name, const pixpg::location& loc) {
      start_time = std::chrono::high_resolution_clock::now();
      pixpg::db::send(name, loc);
    }
    virtual void receive(const std::string& name, const pixpg::location& loc) {
...
main(int argc, char **argv)
{
  ...
  int i = 0;
  while(true) {
    db.send("foo", pixpg::location(++i, 2, 3));
